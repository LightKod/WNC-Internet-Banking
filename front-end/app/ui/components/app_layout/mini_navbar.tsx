    ...
    return (
        <>
            ...
            <Sheet ref={sheetRef} size="md" heading="Menu">
                <div className="flex flex-col gap-y-4 items-center overflow-y-auto">
                    ...
                    {(userRole === "admin" || userRole === "employee") && employeePaths.flatMap(group => group.paths).map((path: any) => (
                        <Link key={`${path.name}`} href={path.href} className={clsx(
                            "p-2 text-gray-500 text-2xl hover:text-gray-950 transition-all duration-300 relative",
                            {
                                "text-gray-950 font-semibold": pathname.includes(path.href)
                    ...
                    {userRole === "admin" && adminPaths.flatMap(group => group.paths).map((path: any) => (
                        <Link key={`${path.name}`} href={path.href} className={clsx(
                            "p-2 text-gray-500 text-2xl hover:text-gray-950 transition-all duration-300 relative",
                            {
                                "text-gray-950 font-semibold": pathname.includes(path.href)
                    ...
                    {otherPaths.flatMap(group => group.paths).map((path: any) => (
                        <Link key={`${path.name}`} href={path.href} className={clsx(
                            "p-2 text-gray-500 text-2xl hover:text-gray-950 transition-all duration-300 relative",
                            {
                                "text-gray-950 font-semibold": pathname.includes(path.href)
