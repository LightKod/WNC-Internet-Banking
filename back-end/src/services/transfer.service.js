...
export const initiateTransfer = async ({ source_account_number, destination_account_number, amount, content, fee_payer, user }) => {
    try {
        ...
        if (!sourceAccount || !destinationAccount) {
            return { status: statusCode.STATUS_ERROR, message: 'Invalid account information' };
        }

        if (sourceAccount.balance < amount) {
            return { status: statusCode.STATUS_ERROR, message: 'Insufficient funds in source account' };
        }
        ...
        return { status: statusCode.STATUS_SUCCESS, data: transaction, message: 'Init transaction success' };
    } catch (err) {
        console.error('Error in initiate transfer service:', err);
        return { status: statusCode.STATUS_ERROR, message: err.message };
    }
};
export const confirmTransfer = async ({ otp_code, transaction_id }) => {
    try {
        ...
        if (!otpRecord) {
            return { status: statusCode.STATUS_ERROR, message: 'Invalid or expired OTP' };
        }
        ...
        if (Date.now() - otpCreatedAt > OTP_EXPIRY_TIME) {
            ...
            return { status: statusCode.STATUS_ERROR, message: 'OTP has expired' };
        }
        ...
        if (!transaction) {
            return { status: statusCode.STATUS_ERROR, message: 'Transaction not found or already processed' };
        }
        ...
        return { status: statusCode.STATUS_SUCCESS, message: 'Transfer completed successfully' };
    } catch (err) {
...
export const initiateExternalTransfer = async ({ source_account_number, destination_account_number, amount, bank_code, content, fee_payer, user }) => {
    try {
        ...
        return { status: statusCode.STATUS_SUCCESS, data: transaction, message: 'OTP sent to email' };
    } catch (err) {
